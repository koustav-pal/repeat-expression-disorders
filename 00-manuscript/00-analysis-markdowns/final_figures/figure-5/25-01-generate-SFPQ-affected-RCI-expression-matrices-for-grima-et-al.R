...
pbmc_object_list <- readRDS("42-grima-sALS-blood-transcriptomics/rdata_files/01-02-list_of_counts_stat_samplesheet_from_grima_et_al.rds")
patient_df <- pbmc_object_list$sample_df
...
counts_matrix <- pbmc_object_list$counts
...
patient_df$Disease_status <- factor(patient_df$Disease_status, levels = c("Control", "sALS"))
patient_df$Sex <- factor(patient_df$Sex, levels = c("F", "M"))

deseq_object <- DESeqDataSetFromMatrix(countData = round(counts_matrix[keep_hash_ids,])[,patient_df$column_id],
    colData = patient_df,
    design = ~ Disease_status + Sex)
...
patient_penetrance_matrix <- do.call(rbind, lapply(seq_len(nrow(patient_df)), function(x){
    temp_df <- patient_df[x,]
...
patient_df$tissue_group <- "blood"
patient_df$penetrance_row <- match(patient_df$column_id, colnames(sfpq_affected_rcis_matrix))

saveRDS(object = list(patient_df = patient_df, 
    ...
    file = file.path(save_dir, "06-01-save-grima-blood-pbmc-sfpq-affected-rci-expression-matrix.rds"))